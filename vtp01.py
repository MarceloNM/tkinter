...
def lerElementos(ficheiro):
    global Elementos
    global maxElementos
    existe = os.path.isfile(ficheiro)
    if existe:
        with open(ficheiro, mode = 'r') as fich:
            csvFile = csv.reader(fich)
            for linhas in csvFile:
                Elementos.append(linhas)
        fich.close()
    maxElementos = len(Elementos)
...
lerElementos(ficheiro)

for i in Elementos:  # elementos
        txt = i[2]
    #print(txt)
        bttnn = tk.Button(qdr, text = txt)
        bttnn.grid(column = i[1], row = i[0], padx = 1, pady = 1)
